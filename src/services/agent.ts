...
export class AgentService {
  ...
  private logMcpServerStatus(msg: { [key: string]: unknown }): void {
    if (!('mcp_servers' in msg) || !Array.isArray(msg.mcp_servers)) {
      return;
    }

    type McpServerInfo = { name: string; status: string; error?: string };
    const mcpServers = msg.mcp_servers as McpServerInfo[];
  ...
  private detectLanguageFromInput(input: unknown): string {
  ...
  private recordWriteToolMetrics(
    input: unknown,
    metricsService: NonNullable<ReturnType<typeof getMetricsService>>
  ): void {
